...
export default function SubmitSpinner() {
  ...
  const handleSubmit = async () => {
    ...
    const dataFetch = await fetchFunc(globalState, 1, 'get');
    if (toggleActivePage === 'report') {
  ...
  if (toggleActivePage === 'report') {
    return (
      <div className="d-flex align-items-end col-sm-4 col-lg-3 col-xl-2 pt-1 pb-3 button_max_width">
        <button
          className="btn btn-outline-primary btn-sm button_max_width"
          type="button"
          style={{ height: '32px' }}
          onClick={handleSubmit}
        >
          {loadingSpinnerReport === true ? (
            <span
